...
const FooterButton = ({ url, title }: { url: string; title: string }) => {
  return (
    <>
      <button onClick={(e) => openLink(e, url)} className="w-full relative group flex items-center">
        ...
        <div className="w-full text-14 sm:text-16 md:text-18 font-azo text-green group-hover:text-green-light pl-0 group-hover:pl-20 transition-all duration-200 text-left">
          {title}
        </div>
      </button>
    </>
